...
public class JapaneseTextField: UITextField {
    ...
    public override var textInputMode: UITextInputMode? {
        if let inputMode = UITextInputMode.activeInputModes.filter({
            (inputMode) -> Bool in
...
extension String {
    ...
    fileprivate func convertText(using chart: CharacterConversionChart.Type)
        -> String
    {
        var text = self
        var position = 0
    ...
    @discardableResult private func convertText(
        _ text: inout String,
        startPosition: Int,
        offset: Int,
        chart: CharacterConversionChart.Type
    ) -> Bool {
...
private protocol CharacterConversionChart {
    static var lookupTable: [String: String] { get }
}

// MARK: - CaseBasedChart
private struct CaseBasedChart: CharacterConversionChart {
    fileprivate static let lookupTable: [String: String] = {
        ...
        for key in katakanaTable.keys {
            katakanaTable[key.uppercased()] = katakanaTable.removeValue(
                forKey: key
            )
        }
        return hiraganaTable.merging(katakanaTable) { (current, _) in
            return current
        }
    }()
}
...
// MARK: - HiraganaChart
private struct HiraganaChart: CharacterConversionChart {
...
// MARK: - KatakanaChart
private struct KatakanaChart: CharacterConversionChart {
